...
void PHP_MD5Init(MD5_CTX *ctx);
void PHP_MD5Update(MD5_CTX *ctx, const void *data, size_t size);
void PHP_MD5Final(unsigned char *result, MD5_CTX *ctx);
...
int md5_file(char *file, char *retval)
{
    int           raw_output = 0;
    char          md5str[33];
    ...
    while ((n = fread(buf, sizeof(buf), 1, stream)) > 0) {
